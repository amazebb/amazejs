...
| `stickyHeaders` | boolean | `true` | Freezes the toolbar at the top of the scroller and pins the table's header row directly beneath it (offset by `--aj-toolbar-h`, the toolbar's measured height). The toolbar also sticks to the container's **left** edge, so the title, the RAW toggle and the menus stay on screen while a table wider than the viewport scrolls sideways. That needs a box narrower than the container it sticks within: the container grows to the table when frozen, so the controller sizes the toolbar itself (`syncToolbarBox`, re-run whenever the toolbar or its host resizes). Sticky pins it to the **scroller's** left edge, so its width must be the scroller's too — `sidewaysScroller` finds the ancestor that scrolls sideways, or the page, which is the normal case since freezing drops `.table-wrap`'s overflow. It is then pulled out to that scroller's edges with a negative `margin-left` and given the same amount back as `padding-left`: the background spans the screen at every scroll position while the title still lines up with the table beneath it. Sized to its host's content box instead — as it was until v0.35.2 — it lands at the scroller's left edge but stops short on the right by exactly the page's own inset (8px of body margin on the demo, 24px of body padding on brewbar), and rows scroll past in that strip above the frozen bar. A **nested** table is the exception: it scrolls inside its parent's cell, not the page, so it keeps its host's content-box width and no bleed. Toggled at runtime from the Settings menu. |
