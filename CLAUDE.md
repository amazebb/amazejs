...
- All table toolbars collapse the same way: only title + count badge show by default, with everything else (export, extra buttons, settings) inside `.atv-toolbar-more`, revealed by an ellipsis overflow button (`.atv-more-btn`): clicking it toggles open/closed (state in `aria-expanded`, visibility via CSS sibling selector) — consistent on mouse and touch, no hover-reveal. New toolbar items should be appended to the `btnHost` container in `buildToolbar` so they collapse automatically.
