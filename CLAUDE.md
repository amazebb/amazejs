...
- A column that is currently filtering says so two ways: the `--accent-subtle` tint on the `<th>` (`.active`), and the `.atv-filter-btn` — hidden on pointer devices until then — appearing as a `--filter-icon` funnel (outline, so it never reads as a filled sort arrow). The tint alone can't say *which* of several columns is filtering; the funnel can, without a glyph in the label text. `.active` is set from the column's own state (a text/range value, or a category selection short of its full value set) and never from the visible option counts, which another column's filter can zero out.
