...
- **`model.js`** — pure functions only, no DOM. Handles data fetching (`fetchData`, `parseTsv`), column inference (`inferColumns`), filtering (`getVisible`, `computeCounts`), and sorting (`sortItems`).
  ...
  **Fetching from a webview's own scheme** — `fetchData` treats a response as delivered when `res.ok` **or `res.status === 0`** (`delivered`). Custom schemes — Electrobun's `views://`, Tauri's `tauri://`/`asset://`, Capacitor's asset protocol — answer a bundled file with a bare response that has no HTTP status, so `res.ok` is `false` while the body is intact; without the status-0 case those loads fell through to the fallback URL and warned. It stays safe because a request that truly failed rejects out of `fetch` rather than arriving here. The same shells are why `application/octet-stream` is not on `BINARY_MIME` and why `parseByUrl` picks the parser from the extension: they usually send no `content-type` at all.
