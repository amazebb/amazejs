...
| `stickyHeaders` | boolean | `true` | Freezes the toolbar at the top of the scroller and pins the table's header row directly beneath it (offset by `--aj-toolbar-h`, the toolbar's measured height). The toolbar also sticks to the container's **left** edge, so the title, the RAW toggle and the menus stay on screen while a table wider than the viewport scrolls sideways. That needs a box narrower than the container it sticks within (the container grows to the table when frozen), and that box is the **scroller's**, since that is where sticky lands it. **The controller measures, the stylesheet lays out.** `syncToolbarBox` (controller.js, re-run whenever the toolbar or its host resizes) publishes three custom properties on the container — `TOOLBAR_VARS` — and nothing else: `--aj-port-w` (the scroller's visible width, from `portBox`), `--aj-port-inset` (how far in the container sits, scroll-invariant), and `--aj-toolbar-h` (the toolbar's measured height). Everything laid out from them is CSS: `.atv-toolbar.atv-sticky` takes `--aj-port-w` as its width, bleeds to the scroller's edges with a negative `margin-left` of `--aj-port-inset` and hands the same amount back as `padding-left` — so the bar spans the screen at every scroll position while the title stays aligned with the table under it — and the header row pins at `top: var(--aj-toolbar-h)`. Keep it that way: a new number goes in as a property, not as an inline style. `portBox` finds the scroller with `sidewaysScroller` (the nearest ancestor with `overflow-x: auto|scroll`, else the page — the normal case, since freezing drops `.table-wrap`'s overflow). Sized to its host's content box instead — as it was until v0.35.2 — the toolbar lands at the scroller's left edge but stops short on the right by exactly the page's own inset (8px of body margin on the demo, 24px of body padding on brewbar), and rows scroll past in that strip above the frozen bar. A **nested** table is the exception `portBox` carries: it scrolls inside its parent's cell, not the page, so its width is that cell's content box and its inset is 0. A fourth property, `--aj-port-pad`, is that same inset again as the frozen container's `padding-right`: overflowing the host's content box overflows its right padding too, so without it a too-wide table runs flush into the window edge while its left keeps the page's inset. It is the inset **only while the table overflows** — the box is border-box, so on a table that already fits the padding would take a bite out of the table's width instead of adding scrollable space. `overflowsHost` zeroes the property before measuring, since a table is `width: 100%` of the box it lands in: measured with the padding still applied, the table would look closer to fitting and the answer would flip back and forth between syncs. Toggled at runtime from the Settings menu. |
