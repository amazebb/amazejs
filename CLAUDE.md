...
- Filter dropdowns are portalled to `<body>` and positioned via JS; they use the native Popover API (`popover="auto"`). `attachPopover` (view.js) wires invoker buttons and keeps their `aria-expanded` in sync.
- `.aj-rotate` is the reusable indicator-rotation utility: any element with the class spins its `::before`/`::after` arrow while `aria-expanded="true"` (angle via `--aj-rotate-angle`, default 180deg). Used by column filter chevrons, the export split arrow, and tree row toggles (`.aj-toggle`, 90deg); apply it to future toggling UI rather than writing new transitions.
