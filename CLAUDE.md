...
For CDN consumers there is **one** build: `bun run build` bundles `build/entry.js` into `dist/amazejs.js` — a single self-contained, minified ESM file with `amazejs.css` inlined as a `<style>`. Consumers pin an exact tag — `cdn.jsdelivr.net/gh/amazebb/amazejs@v0.40.0/dist/amazejs.js` — never `@latest`, which browsers cache for 7 days and no purge can clear. A release is: `bun run build`, commit `dist/`, tag `vX.Y.Z` (feat bumps the minor, fix the patch), push the branch and the tag (lightweight tags need `git push origin <tag>`; `--follow-tags` skips them), then bump the pin in `docs/app.js`, in `README.md`'s three examples, and in `../brewbar/docs/app.js`. Each tag is a fresh immutable URL, so no purge is needed. There is no lint or test command.
...
- **`model.js`** — pure functions only, no DOM. Handles data fetching (`fetchData`, `parseTsv`), column inference (`inferColumns`), filtering (`getVisible`, `computeCounts`), and sorting (`sortItems`).
  ...
  **Fetching from a webview's own scheme** — `fetchData` counts a response as delivered when `res.ok` **or `res.status === 0`** (`delivered`). Custom schemes — Electrobun's `views://`, Tauri's `tauri://`/`asset://`, Capacitor's assets — answer a bundled file with a bare response carrying no HTTP status, so `res.ok` is `false` while the body is intact, and without this those loads fell through to the fallback URL. It stays safe because a request that truly failed rejects out of `fetch` rather than arriving here. Those schemes also tend to send no `content-type`, which is why the extension is consulted ahead of it.

  **Is the first row a header?** — `hasHeaderRow(rows)` decides for a delimited file, `toRecords` applies it, and `hasGeneratedHeaders(data)` reads back which way it went (the `col1..colN` convention lives beside the code that writes it). See the `headerRow` option below.

  **Choosing a parser** — `formatFor(name, mimeType, text)` returns `'json'`, `'csv'` or `'tsv'`, and `parseAs` turns that into rows; both readers share them, as they share `unreadableReason`. Three tests, in order: an extension that names a format (`.json`/`.csv`/`.tsv`) — the author's own label, and the one thing a redirect or a proxy can't restate; then the served media type (`text/plain` is deliberately not on that list, since servers hand it out for CSV and TSV alike); then the content — a leading `{` or `[` is JSON, otherwise whichever of tabs and commas the first line holds more of, ties going to TSV. Only the first test existed before, so a name that says nothing about the body — an API path (`/v1/datasets/42`), a query-string export (`?format=csv`), or an extension naming the server rather than the file (`report.php`) — fell through to TSV and built a one-column table of whole CSV lines, silently.
...
|---|---|---|---|
| `data` | `Array`, root object, or `[url, fallbackUrl?]` | required | If a string array, fetched via `fetchData` and parsed by `formatFor` (see `model.js`), the second URL used only if the first request fails |
...
`content-box` on the container is what makes the trailing padding unconditional: the padding is added outside the width, so a table that already fits keeps its full width (the padding lands in the host's own padding and scrolls nothing) while a too-wide one gets the space after it. Under `border-box` the same declaration bites into the width instead, so the frozen container opts back out of the `border-box` the component sets on itself and everything in it.
...
| `showToolbarControls` | boolean | `true` | `false` hides `.atv-toolbar-rest` — the count badge, the RAW toggle, the `⋯` and every menu behind it — leaving the toolbar as the disclosure handle and title alone. The same wrapper collapsing the table hides, so the two use one mechanism. Distinct from `showToolbar` (no toolbar at all) and `showButtons` (menus out permanently vs behind the `⋯`). Named `showFilterRow` until v0.38.0, which said nothing about what it did. |
...
| `headerRow` | `'auto'`, `true`, `false` | `'auto'` | Whether a CSV/TSV's first row names the columns. A delimited file doesn't say, and reading a data row as the header costs that row, labels the columns with values, and — when one of them is integer-like — reorders them, since JS hoists integer-like keys to the front of an object. `'auto'` weighs the first row against the rest per column (`hasHeaderRow`, model.js): a column whose body is uniformly numbers or dates has a header when its first cell isn't one, and is all data when it is; a first row repeating a value is data too, names having to be distinct to key a record by. Undecided means header — what nearly every delimited file has, and what this always assumed. A file of nothing but strings gives the test no signal, which is what **Settings > First Row is Header** is for: it re-parses `sourceText`'s kept text the other way and rebuilds, so the row that appears or disappears gets its say in which columns are numeric and which filters they earn. The row shows only for a delimited import; JSON has no such question. Headerless columns are keyed `col1..colN` and carry no label of their own, so `labelFor` names them like any other (`Col1`, or `COL1` in a tree). |
...
- All dropdowns are nested in their trigger's DOM (filter dropdowns inside the `<th>`, array dropdowns inside the `<td>`, File/settings dropdowns in the toolbar) — never portalled to `<body>`. The native Popover API (`popover="auto"`) renders them in the top layer when open, and DOM nesting means File > Open can rebuild a table by replacing its container without leaking dropdowns. **Placement is written twice, once per engine** — the two `@supports` blocks on `.filter-dropdown` in amazejs.css, picked between by `ANCHOR_CSS` in view.js. A popover is fixed and in the top layer, so it is placed against the viewport and what the viewport cuts off is gone, not scrolled to: without the flip a `translations` cell on the last row shows one pair of nineteen.

  *With CSS anchor positioning* (Chromium, Safari 26): `attachPopover` gives the trigger an `anchor-name` and the dropdown a matching `position-anchor`, and `position-area: block-end span-inline-end` makes the room below the trigger the dropdown's containing block — so it opens under the trigger's left edge, `max-height: clamp(120px, calc(100% - margins), 360px)` caps it to the room on that side (the cap only shrinks; the floor keeps it from becoming a sliver), and `position-try-fallbacks` + `position-try-order: most-block-size` flip it to whichever side has more room. Because the cap means it never overflows, `most-block-size` is what triggers the flip — and it flips toward the roomier side even when the dropdown would have fitted below. Nothing measures in JS.

  *Without it* — **WKWebView**, so Electrobun, Tauri, and any app embedding a web view, plus Safari before 26. Anchor positioning is an experimental WebKit feature that is off by default there and can't be turned on from the page; a page-level fallback is the only fix. Unplaced, `position: fixed; inset: auto` leaves each popover at its *static* position, uncorrected for scroll — which is why the toolbar menus draw over the toolbar, and why a filter works on the columns visible at load and then opens off-screen for any column you had to scroll right to reach. So `place()` in view.js measures the trigger with `getBoundingClientRect()` and publishes its distance to each of the four viewport edges as `--aj-a-t/-b/-l/-r`, plus `data-dd-y`/`data-dd-x` for the side it picked; the stylesheet does the arithmetic, keeping the gap, the edge inset, the height cap and the flip. It re-places on scroll and resize while open, since a fixed box doesn't follow its trigger. `mvp.html` in the repo root is a standalone 50-column test page that compares both mechanisms side by side. Don't enlarge the JS side: a new number goes in as a custom property, the layout stays in CSS.

  `attachPopover` (view.js) wires invoker buttons and keeps their `aria-expanded` in sync; with `{ hover: true }` (File/settings buttons and column-filter `<th>`s) the dropdown also opens on pointer-over and closes after a grace delay once the pointer leaves both invoker and dropdown (unless a text input inside is focused); moving onto a different column header closes it immediately. Column filter dropdowns have no trigger button: the `<th>` itself is the invoker — hover opens the filter, click sorts.
